 ...
 *        -1 on error
 */
void print_err(info_t *_Info_, char *estr)
{
	_eputs(_Info_->fname);
	_eputs(": ");
	print_d(_Info_->count_line, STDERR_FILENO);
	_eputs(": ");
	_eputs(_Info_->argv[0]);
	_eputs(": ");
	_eputs(estr);
}
...
char *convert_num(long int num, int base, int flags)
{
	...
	array = flags & CONV_LC ? "0123456789abcdef" : "0123456789ABCDEF";
	ptr = &buffer[49];
	*ptr = '\0';
